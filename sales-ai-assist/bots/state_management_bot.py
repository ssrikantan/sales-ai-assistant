...
class StateManagementBot(ActivityHandler):
    ...
    '''Create a new Vector database using the Assistant API v2. 
    It Uploads the documents, chunks them and creates the vector database.
    After this is done, it updates the assistant with the new vector database id.
    The index is now ready for File-search tool in the Assistant API v2.
    '''
    def create_vector_database(self) -> str:

        # Create a vector store called "SOW-Archives-v1"
        vector_store = StateManagementBot.client.beta.vector_stores.create(name="SOW-Archives-v1")
        ...
        StateManagementBot.client.beta.assistants.update(
    ...
    def update_vector_database(self):
        ...
        print(file_batch.status)
        print(file_batch.file_counts)
        assistant = StateManagementBot.client.beta.assistants.update(
            assistant_id=self.config.assistant_id,
            tool_resources={"file_search": {"vector_store_ids": [self.config.vector_store_id]}},
        )
